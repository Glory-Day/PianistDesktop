...
def plot_one_box(x, im, color=None, label=None, line_thickness=1):
...
def plot_one_box_PIL(box, im, color=None, label=None, line_thickness=None):
    ...
    if label:
        fontsize = max(round(max(im.size) / 100), 12)
...
def profile_idetection(start=0, stop=0, labels=(), save_dir=''):
    # Plot iDetection '*.txt' per-image csv. from utils.plots import *; profile_idetection()
